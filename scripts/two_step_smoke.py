"""Minimal smoke command for the Stage-2 two-step scaffold."""

from __future__ import annotations

import argparse
import logging
import sys
from pathlib import Path

import numpy as np

REPO_ROOT = Path(__file__).resolve().parents[1]
SRC_DIR = REPO_ROOT / "src"
if str(SRC_DIR) not in sys.path:
    sys.path.insert(0, str(SRC_DIR))

from sisinf.two_step import (  # noqa: E402
    DiagnosticReducedRowBackend,
    TwoStepReductionTarget,
    run_two_step_on_reduced_basis,
)
from sisinf.types import Instance  # noqa: E402


def _tiny_instance() -> Instance:
    return Instance(
        name="two_step_smoke",
        index=0,
        n=2,
        m=2,
        q=7,
        gamma=7,
        A=np.array([[1, 2], [3, 4]], dtype=np.int64),
        t=np.zeros(2, dtype=np.int64),
        require_l2_ge_q=False,
        homogeneous=True,
        source_path=Path("memory"),
    )


def main(argv: list[str] | None = None) -> int:
    """Run the smoke CLI."""

    parser = argparse.ArgumentParser(description="Stage-2 two-step scaffold smoke command.")
    parser.add_argument("--kappa", type=int, default=2, help="Projected sublattice dimension. Default: 2.")
    parser.add_argument("--top-k", type=int, default=2, help="Diagnostic backend output cap. Default: 2.")
    parser.add_argument("--verbose", action="store_true", help="Enable INFO logs.")
    args = parser.parse_args(argv)

    logging.basicConfig(level=logging.INFO if args.verbose else logging.WARNING, format="[%(levelname)s] %(name)s: %(message)s")

    inst = _tiny_instance()
    reduced_basis = np.array(
        [
            [-1, -3, 1, 0],
            [-2, -4, 0, 1],
            [7, 0, 0, 0],
            [0, 7, 0, 0],
        ],
        dtype=np.int64,
    )
    result = run_two_step_on_reduced_basis(
        inst,
        reduced_basis,
        reduction_target=TwoStepReductionTarget(beta=2, target_rhf=None, max_loops=1),
        kappa=args.kappa,
        backend=DiagnosticReducedRowBackend(top_k=args.top_k),
    )

    print(f"backend: {result.backend_name}")
    print(f"diagnostic_only: {result.backend_diagnostic_only}")
    print(f"kappa: {result.projected_sublattice.kappa}")
    print(f"projected_representation: {result.projected_sublattice.representation}")
    print(f"vector_count: {len(result.vectors)}")
    for idx, vec in enumerate(result.vectors, start=1):
        print(f"vector[{idx}]: {vec.tolist()}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
